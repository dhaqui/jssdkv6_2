...
// Render では同一オリジンから配信するので CORS 制限は不要
// ローカル開発時のみ許可
app.use(cors({
  origin: NODE_ENV === "production" ? false : "http://localhost:8080",
}));
...
const {
  ...
  PORT = 8080,           // Render は $PORT を自動で注入する
...
// ----------------------------------------------------------------
// 静的ファイル配信（HTML / CSS / JS を同一サーバーから配信）
// ----------------------------------------------------------------
